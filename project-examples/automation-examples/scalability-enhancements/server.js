...
const logger = winston.createLogger({
  ...
  defaultMeta: { service: 'scalability-enhancements' },
  transports: [
...
// Import routes
const scalabilityRoutes = require('./routes/scalability');
...
app.get('/health', (req, res) => {
  res.json({
    status: 'healthy',
    service: 'scalability-enhancements',
    timestamp: new Date().toISOString(),
...
if (cluster.isMaster) {
  ...
  app.listen(PORT, () => {
    logger.info(`Scalability Enhancements service running on port ${PORT} (Worker ${process.pid})`);
  });
}
