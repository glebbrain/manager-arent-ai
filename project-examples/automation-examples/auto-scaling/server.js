...
const logger = winston.createLogger({
  ...
  defaultMeta: { service: 'auto-scaling' },
  transports: [
...
const metricCollector = require('./modules/metric-collector');
const scalingPolicy = require('./modules/scaling-policy');
const loadBalancer = require('./modules/load-balancer');
const healthChecker = require('./modules/health-checker');
const capacityPlanner = require('./modules/capacity-planner');
...
if (cluster.isMaster) {
  ...
  app.listen(PORT, () => {
    logger.info(`Auto-scaling service running on port ${PORT} (Worker ${process.pid})`);
  });
}
