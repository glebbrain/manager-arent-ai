...
try {
    ...
    $psi.FileName = 'python'
    $psi.Arguments = '-m pip_audit --version'
    ...
    $p.WaitForExit()
    if ($p.ExitCode -ne 0) {
        $summary = @{ tool='sca_pip_audit'; available=$false; exitCode=0; note='pip-audit not installed'; timestamp=(Get-Date).ToString('s') }
        Write-JsonSummary -Summary $summary -Path $JsonOut
        if (-not $Quiet) { Write-Host 'sca_pip_audit: skipped (pip-audit not installed)' }
        exit 0
    }

    $psi2 = New-Object System.Diagnostics.ProcessStartInfo
    $psi2.FileName = 'python'
    $psi2.Arguments = '-m pip_audit -f json'
    ...
    Write-JsonSummary -Summary $summary -Path $JsonOut
    if ($JsonOut) { $stdout | Set-Content -LiteralPath ($JsonOut -replace '\.json$', '.pip_audit.json') -Encoding UTF8 }
    if (-not $Quiet) { Write-Host "sca_pip_audit: exit=$code" }
    ...
    Write-JsonSummary -Summary @{ tool='sca_pip_audit'; exitCode=1; error=$_.Exception.Message } -Path $JsonOut
    exit 1
}
