...
try {
    $exists = Test-Path -LiteralPath $Spec
    $summary = [ordered]@{
        tool      = 'api_contract_test'
        specPath  = $Spec
        exists    = $exists
    ...
    if (-not $Quiet) { Write-Host "api_contract_test: exists=$exists ($Spec)" }
    ...
    Write-JsonSummary -Summary @{ tool='api_contract_test'; exitCode=1; error=$_.Exception.Message } -Path $JsonOut
    exit 1
}
